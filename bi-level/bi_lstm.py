...
class LSTM(base.NN):
    ...
    VOCABULARY_SIZE = load.Data.VOCABULARY_SIZE     # 词典大小
    NUM_NODES = 64                                  # layer1 有多少个节点

    SHAPE_DATA = [BATCH_SIZE, ]                     # 输入 data 的 shape
    SHAPE_INPUT = [EMBEDDING_SIZE, NUM_NODES]       # cell 中与输入相乘的权重矩阵的 shape
    ...
    def init(self):
        ...
        for _ in range(self.NUM_UNROLLINGS + 1):
            place_input = tf.placeholder(tf.int32, shape=self.SHAPE_DATA, name='input')
            self.__trainData.append(place_input)

            embedding = tf.nn.embedding_lookup(self.__embeddings, place_input, name='embed_input')
            self.__trainEmbed.append(embedding)

            place_label = tf.one_hot(place_input, self.VOCABULARY_SIZE, name='label')
            self.__trainLabel.append(place_label)

        self.__X = self.__trainEmbed[:self.NUM_UNROLLINGS]
        self.__y = self.__trainLabel[1:]                       # label 为 输入数据往后移一位 (序列数据)
    ...
    def load(self):
        # 加载 embeddings
        self.__embeddings = tf.Variable(load.Embedding.load(), dtype=tf.float32, name='embeddings')
    ...
    def __sample(prediction):
        return LSTM.__sampleDistribution(prediction[0])
    ...
    ''' 将 batches 转换为 one_hot_batches '''
    def oneHot(self, batches):
        one_hot_batches = []
        for batch in batches:
            one_hot_batches.append( self.sess.run( tf.one_hot(batch, self.VOCABULARY_SIZE) ) )
        return one_hot_batches
