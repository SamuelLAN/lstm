...
'''
class CBOW(embedding.Embedding):
    MODEL_NAME = 'cbow'         # 模型的名称

    # 模型存储的路径，默认使用 cbow 模型
    SAVE_PATH = r'../data/text8_bi_embedding_cbow.pickle'


    ''' 初始化 X 与 y 的接口 （ 被 self.init() 调用 ） '''
    def initXY(self):
        # 输入 与 label
        self.__X = tf.placeholder(tf.int32, [self.BATCH_SIZE, 2 * self.SKIP_WINDOW])  # 训练集数据
    ...
    def model(self):
        with tf.name_scope('layer_1'):
            ...
            embed = tf.nn.embedding_lookup(self.__embeddings, self.__X, name='multi_embed')
            self.__embed = tf.reduce_mean(embed, 1, name='embed')
    ...
    def run(self):
        ...
        for step in range(self.steps):
            ...
            batch_x, batch_y = self.data.cBOWNextBatch(self.BATCH_SIZE)
            ...
            if step % self.iterPerEpoch == 0 and step != 0:   # 完成一个 epoch 时
                ...
                # 当前 loss 比 最好的 loss 低，则更新 best_loss
                if loss < best_loss:
                    best_loss = loss
